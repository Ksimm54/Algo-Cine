...
-------------------------------------------------------------------------------
                    430 Régression Lasso
-------------------------------------------------------------------------------
"""

def etape430():

    etape = "430_regressionLasso"
    ...
    # Définition et entraînement du modèle Lasso
    alpha = 0.1  # Paramètre de régularisation à ajuster
    lasso = Lasso(alpha=alpha, random_state=123)
    lasso.fit(X_train, y_train)
    
    # Prédictions
    y_train_pred = np.round(lasso.predict(X_train)).astype(int)
    y_val_pred = np.round(lasso.predict(X_val)).astype(int)
    y_test_pred = np.round(lasso.predict(X_test)).astype(int)
    ...
    films_pred.to_excel(os.path.join(output_folder_400, 'regressionLasso_Films_pred.xlsx'), index=False)
  
    
    # Affichage des coefficients
    coef_df = pd.DataFrame({'Feature': features, 'Coefficient': lasso.coef_})
    coef_df = coef_df[coef_df['Coefficient'] != 0]  # On garde que les variables sélectionnées
